...
as_esri_features <- function(x, crs = NULL, call = rlang::caller_env()) {
  # class check
  valid_sfg_classes <- c(
    "sf",
    "data.frame",
  ...
  switch(x_class,
    "sf" = as_esri_features_sf(x, crs, call = call),
    "data.frame" = as_esri_features_sf(x, crs, call = call),
    "sfc" = as_esri_features_sfc(x, crs, call = call)
  )
}
...
as_esri_features_sf <- function(x, crs = NULL, call = rlang::caller_env()) {
  ...
  if (three_dim) {
    sf_as_features_3d(.data, .geoms, nrow(.data), sr)
  } else {
    sf_as_features_2d(.data, .geoms, nrow(.data), sr)
  }
}
