...
/// @export
/// @rdname features
fn sfc_polygon_features_2d(x: List, sr: Robj) -> String {
    let sr = deserialize_sr(&sr);
    let res = SfcPolygon(x).as_features_2d(sr).unwrap();
...
/// @export
/// @rdname features
fn sfc_polygon_features_3d(x: List, sr: Robj) -> String {
    let sr = deserialize_sr(&sr);
    let res = SfcPolygon(x).as_features_3d(sr).unwrap();
    serde_json::to_string(&res).unwrap()
}
